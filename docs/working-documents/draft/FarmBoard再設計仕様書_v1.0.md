...
| US-04 | 担当者 | 翌日、自身のFarmBoardを開くと、前日に牧場長から割り当てられたタスクだけが一覧で表示されている。 | 今日やるべき作業が明確になり、指示を待つことなくすぐに業務を開始できる。 |
| US-05 | 担当者 | タスクの完了後、FarmBoard上のチェックボックスをチェックするだけで、簡易的に完了報告ができる。 | 事務所に戻って報告する手間がなくなり、作業の合間にリアルタイムで進捗を更新できる。 |
| US-06 | 牧場長 | FarmBoardから、Todayセクションにアクセスし、牧場全体の全タスクの進捗状況（担当者、ステータス等）をリアルタイムで俯瞰的に確認できる。 | 個々のスタッフに状況を聞いて回らなくても、牧場全体の作業の進捗や遅延を把握し、ボトルネックがあればすぐに介入できる。 |
...
### 5.2. パーソナライズド・タスクリスト（担当者向けビューのメインコンテンツ）
- **役割:** 担当者がその日に実行すべき作業を明確に提示し、実行を支援する。
- **表示ロジック:**
    - 前日に管理者（牧場長）によって割り当てられた、自分担当のタスクのみをリスト形式で表示する。
    - これが担当者向けビューのメインコンテンツとなる。
- **機能:**
    - **タスクの確認:** タスク内容、対象牛、申し送り事項などを確認できる。
    - **完了報告:** 各タスクの横にあるチェックボックスをチェックすることで、簡易的にタスクを完了できる。
    - **詳細報告:** 音声入力やテキスト入力により、活動の詳細（使用薬品など）を記録できる。
- **詳細仕様:** 本コンポーネントのインタラクションや振る舞いの詳細については、`Today機能再設計仕様書_v1.0.md` で定義する。

### 5.3. Todayセクションへのアクセス（管理者向けビュー）
- **役割:** 管理者が牧場全体のタスク進捗を把握するための入り口。
- **表示ロジック:**
    - 管理者向けビューでは、タスクのサマリー（全タスク数、完了数、未完了数など）をウィジェットとして表示。
- **機能:**
    - ウィジェットをクリックすると、管理者向けの「Todayセクション」（詳細なタスク管理画面）に遷移する。詳細な仕様は `Today機能再設計仕様書_v1.0.md` を参照。
...
- **パーソナライズド・タスクリスト:** 本機能の担当者向けビューは、`Today機能再設計仕様書_v1.0.md`で定義された機能と密に連携する。
