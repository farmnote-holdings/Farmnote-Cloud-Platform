# **Farmnote Cloud Figmaデザインガイドライン Ver.1.01**

**最終更新日:** 2025年07月17日 21:56
...
### **7.3. Base Component + Wrapper パターン (限定的な利用)**

`MenuItem`や`TabItem`のような最小単位の「基本部品（Base Component）」をまず定義し、それを`Menu`や`Tabs`といった「ラッパー（Wrapper）」コンポーネントで束ねて使用するパターンです。

*   **役割:** リスト形式のUIで、アイテムの数や種類が可変な場合に有効です。
*   **使い方の例:**
    *   **Menu:** `MenuItem`というコンポーネントを作成し、Auto Layoutが設定された`Menu`フレームの中に必要な数だけ配置して使います。
    *   **Tabs:** `TabItem`コンポーネントを作成し、`Tabs`フレームに並べます。

**注意点とリスク:**
このパターンは柔軟性が高い一方で、ラッパー（`Menu`など）の中の構成が固定されないため、**インスタンスごとの再現性が低くなるという重大なリスク**を伴います。安易に採用すると、UIの一貫性が損なわれる可能性があります。

**利用シーン:**
このパターンの採用は、`Tabs`のようにアイテム数が少なく構成がシンプルな場合に限定し、それ以外の複雑なレイアウトを持つコンポーネントには、前述の**Slotパターンを使い、差し替え可能な領域を明確に定義することを強く推奨します。**
